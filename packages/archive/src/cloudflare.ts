...
export class CloudflareArchiveRepository implements ArchiveRepository {
  ...
  async exportAuditBundle(input: { start?: string; end?: string; merkleRoot?: string }): Promise<unknown> {
    const reads = await this.db
      ...
      .all<Array<Record<string, unknown>>>()
    const readRows = (reads as { results: Array<Record<string, unknown>> }).results
    ...
    const messages = await this.db
      ...
      .bind(input.start ?? null, input.start ?? null, input.end ?? null, input.end ?? null)
      .all<Array<Record<string, unknown>>>()
    const messageRows = (messages as { results: Array<Record<string, unknown>> }).results
    const messageIds = messageRows.map((r) => String(r.id))

    let transitionRows: Array<Record<string, unknown>> = []
    let sealRows: Array<Record<string, unknown>> = []
    if (messageIds.length > 0) {
      ...
      const transitions = await this.db
        ...
        .bind(...messageIds)
        .all<Array<Record<string, unknown>>>()
      transitionRows = (transitions as { results: Array<Record<string, unknown>> }).results

      const seals = await this.db
        .prepare(
        ...
        .bind(...messageIds)
        .all<Array<Record<string, unknown>>>()
      sealRows = (seals as { results: Array<Record<string, unknown>> }).results
    }
    ...
    for (const messageId of messageIds) {
      const proof = await this.getMerkleProof(messageId)
      if (proof) proofs.push(proof)
    }

    const normalizedReads = readRows.map((row) => ({
      id: Number(row.id),
    ...
    const normalizedMessages = messageRows.map((row) => {
      ...
      return {
        ...
        submitted_at: String(row.submittedAt ?? row.submitted_at ?? ''),
    ...
    const normalizedSeals = sealRows.map((row) => ({
      message_id: String(row.message_id),
